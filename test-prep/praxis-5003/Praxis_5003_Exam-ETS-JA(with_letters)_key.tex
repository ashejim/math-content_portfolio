...
\setenumerate[2]{label={}}%%%changes enumii to capital letters 

\newcommand{\questMulti}{ \textbf{Answer the question below by }\clicking\textbf{the correct response.} \\ \\~}

\newcommand{\questMultiPic}{\textbf{Answer the question below by }\clicking\textbf{the correct response.}}
...
\newcommand{\questInd}{\textbf{Indicate all of your answers.} \\ \\}
...
\newcommand{\examTextSize}{\small}
\examTextSize
\newcommand{\multi}{\cdot}
\newcommand{\xspace}{\hspace*{0pt}}
\newcommand{\yspace}{\vspace{0pt} \vfill} %1.4cm
...
%\newcommand{\ans}[1]{\textbf{\textcolor{readablegreen}{#1}} \checkmark} 
\newcommand{\ans}{\textbf{\textcolor{readablegreen}\checkmark}}
%\renewcommand{\ans}{} 
...
%%%options%%
\renewcommand{\textSize}{\small}
%\tiny \scriptsize \footnotesize \small \normalsize, \large, \Large, \LARGE, \huge, \Huge
\renewcommand{\examTextSize}{\normalsize}
\examTextSize
...
\begin{enumerate}
\item[\textbf{C.}] Rectangular pyramid \yspace \vfill
\end{enumerate}
\item \questMultiPic 
\begin{figure}[H]
\begin{center}
...
\begin{enumerate}
\item[\textbf{B.}]  \ans When the numerator stays the same and the denominator increases,
the fraction decreases.\yspace \vfill
\end{enumerate}
\item \questMultiPic 
\begin{figure}[H]
\begin{center}%
...
\begin{tikzpicture}[scale=.75]
	...
	\draw[thick,fill=gray!50!white] \A--\B--\F--\C--\D--\E--\G--cycle;
	
	\draw[fill=black] \A circle (0.1) node[above] {$A(-4,3)$};
	\draw[fill=black] \B circle (0.1) node[above right] {$B$};
	\draw[fill=black] \C circle (0.1) node[above] {$C$};
	\draw[fill=black] \D circle (0.1) node[below] {$D(3,-3)$};
	\draw[fill=black] \E circle (0.1) node[below left] {$E$};
	\draw[fill=black] \F circle (0.1) node[above right] {$F$};
	\draw[fill=black] \G circle (0.1) node[below left] {$G$};
...
\end{tikzpicture}\end{center}
\end{figure}
\noindent In the figure shown, the quadrilateral $ABFG$ is a rectangle
and the quadrilateral $FCDE$ is a square. Which of the following
statements must be true about the figure?\\
\\
Select \textbf{all }that apply.

\begin{enumerate}
\item  \ans $E$ lies on the y -axis.
\item $B$ lies on the $x$-axis.
\item $A$ is in the first quadrant.
\item  \ans $D$ is in the fourth quadrant
\item  \ans The area of the polygon $ABCDEFG$ is 27 square units.
...
\begin{enumerate}
\item  \ans Coefficients: $4,7,-6$; Degrees: $6,4,0$
...
\item Quadrant II
\item Quadrant III
\item  \ans Quadrant IV\yspace \vfill
\end{enumerate}
\item \questMultiPic 
\begin{figure}[H]
...
digit 9 in Edwin's number represents how many times what digit 9 represents
in Tess's number?
...
\item  \ans Irregular and concave\yspace \vfill
\end{enumerate}
\item \questMultiPic  
\begin{figure}[H]
\begin{center}%
...
\item  \ans Because of the real-world context, the solution must belong
...
\item  \ans Brent's, Adeline's, and Chad's\yspace \vfill
\end{enumerate}
...
found.
\item  \ans $\ell$ is the dependent variable because the volume is dependent
...
\item Five thousand
\item Ten thousand
\item  \ans Fifty thousand\yspace \vfill
\end{enumerate}
...
\item  \ans $\frac{1}{5}\times\mbox{3,750}$\yspace \vfill
\end{enumerate}
...
\begin{enumerate}
\item  \ans The distributive property of multiplication over addition.
...
\item $2(b+5)^{2}$
\item $2^{2}(b+5)$
\item  \ans $2^{2}(b+5)^{2}$\yspace \vfill
\end{enumerate}
...
year
\item  \ans The country that had the greatest yearly average MPG for each
of the three years shown had a three year total between 55 and 65
MPG.
\item  \ans The average MPG in China increase by more than one third from
2000 to 2010.
...
\begin{enumerate}
\item  \ans $(306\div2)\div9$
\item  \ans $2(153\div18)$
\item $(153\div9)+(153\div9)$
\item  \ans $(260\div18)+(46\div18)$\yspace \vfill
\end{enumerate}
\item \questMultiPic 
\begin{figure}[H]
\begin{center}%
...
\item  \ans 9 hours and 36 minutes\yspace \vfill
\end{enumerate}
...
of Figure 1 is equal to the perimeter of Figure 2.
\item  \ans The area of Figure 1 is equal to the area of Figure 2, and
the perimeter of Figure 1 is less than the perimeter of Figure 2.\yspace \vfill
\end{enumerate}
\end{enumerate}
