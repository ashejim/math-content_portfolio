...
\renewcommand{\date}{January $15^{\text{th}}$ 5pm-7pm, 2012} %%\today
...
notation.\vfill
\item Write out the following sum ${\displaystyle \sum_{n=0}^{3}(n+1)^{2}}$.\vfill
\end{enumerate}
\emph{Evaluate the geometric series or state that it diverges. }
\begin{enumerate}[resume]
\item ${\displaystyle \sum_{k=0}^{\infty}\frac{1}{4^{k}}}$\vfill
\item ${\displaystyle \sum_{m=0}^{\infty}\frac{(-3)^{2n}}{5^{n}}}$\vfill
\end{enumerate}
...
\begin{enumerate}[resume]
\item ${\displaystyle \sum_{k=0}^{\infty}\frac{k}{3k+2}}$\vfill\newpage
\end{enumerate}
\emph{Use the Ratio Test to determine whether the following series
converges, diverges, or state that the test is inconclusive.}
\begin{enumerate}[resume]
\item ${\displaystyle \sum_{k=1}^{\infty}\frac{1}{k!}}$ (recall that $\frac{k!}{(k+1)!}=\frac{1}{k+1}$).\vfill
\item Let $f(x)=3e^{-x}$. (Each part is worth 4 points.)

\begin{enumerate}[resume]
\item Find the Taylor polynomial of order $n=2$ for $f(x)$ centered at
$0$.\vfill
\item Use the Taylor polynomial from above to approximate $3e^{0.1}$.\vfill
...
\begin{enumerate}[resume]
\item {[}2 pts{]} Set one:\vspace{1cm}
\item {[}2 pts{]} Set two:\vspace{1cm}
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}[resume]
\item $\left(2,\frac{\pi}{3}\right)$\vfill
\end{enumerate}
...
following problems. }
\begin{enumerate}[resume]
\item $\mathbf{u}+\mathbf{v}$\vfill
\item $\mathbf{u\cdot}\mathbf{v}$\vfill
\item Find the angle between $\mathbf{u}$ and $\mathbf{v}$. ($|\mathbf{u}|=\sqrt{3}$
and $|\mathbf{v}|=2\sqrt{5}$.) \vfill
\item Find a vector perpendicular to $\mathbf{u}$ and $\mathbf{v}$.\vfill
\end{enumerate}
\newpage
\begin{enumerate}[resume]
...
horizontal components of the canoe's velocity.\vfill
\end{enumerate}
\emph{Find the indicated partial derivative of the following functions.}
\begin{enumerate}[resume]
\item Find $f_{x}$ and $f_{y}$ for $f(x,y)=5x\cdot\sin2xy$.\vfill
\item Find the four second partial derivatives of $f(x,y)=2x^{5}y^{2}+2x^{2}$.\vfill
\end{enumerate}
...
\begin{enumerate}[resume]
\item $dW/dt$ where $W=x^{2}+y^{2}$, $x=\cos2t$, and $y=\sin2t$.\vfill
\end{enumerate}
\newpage
...
\begin{enumerate}[resume]
\item $f(x,y)=e^{2xy}$; $P(2,3)$\vfill
\end{enumerate}
\emph{Find a vector that points in a direction of no change in the
function at P.}
\begin{enumerate}[resume]
\item $\ln(1+xy)$; $P(2,3)$\vfill
\end{enumerate}
