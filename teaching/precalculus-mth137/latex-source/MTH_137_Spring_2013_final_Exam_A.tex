...
\item $C=Ax+By$ for $y$.\vfill
\item $x^{2}-11=0$\vfill\columnbreak
\item $\frac{z+2}{z-3}=\frac{5}{-3}$\vfill
...
\item Find the center and the radius of the circle. Then graph the circle
$(x-2)^{2}+(y+4)^{2}=16$.\\
...
\end{tikzpicture}
\end{enumerate}
\emph{For problems \ref{enu:imag-first}-\ref{enu:imag-last}, perform
the indicated operations and write your answers in the form $a+bi$.}
\begin{enumerate}[resume]
\item $(2-i)(4+8i)$\label{enu:imag-first}\vfill
\item Simplify the following: \label{enu:imag-last}

\begin{enumerate}
\item $i^{4}$\dspace
\item $i^{25}$\newpage
\end{enumerate}
\item Let $h=\left\{ \left(1,2\right),\left(2,2\right),\left(3,2\right),\left(4,2\right)\right\} $
be a relation assigning $x$ coordinates with $y$ coordinates.

\begin{enumerate}
\item Domain:
\item Range:
\item Is $h$ a function of $x$?\dspace
\end{enumerate}
\end{enumerate}
\emph{Let $f(x)=x+1$ and $g(x)=x^{2}-x$. Find and simplify the expression. }
\begin{enumerate}[resume]
\item $g(-2)$\vfill
...
\item Find the domain and range of $g(x)$.\dspace
\item Graph the inverse of $g(x),$$g^{-1}(x)$.\vfill\columnbreak
\item []\renewcommand{\xmax}{10}
\renewcommand{\xmin}{-10}
\renewcommand{\xstep}{0} %must be xmin+n
\renewcommand{\ymax}{10}
...
\begin{enumerate}
\item []$\log_{11}\left(11y^{2}\right)$\vfill
\end{enumerate}
...
\end{enumerate}
\item Find and graph the equation of the parabola with focus $(-1,6)$ and
directrix $y=2$.\\
