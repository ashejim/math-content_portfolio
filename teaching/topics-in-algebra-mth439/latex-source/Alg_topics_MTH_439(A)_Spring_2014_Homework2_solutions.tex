...
Choose any 10 problems from the following exercises in chapter 2 of
the notes.
...
$f$ is \uline{not} a homomorphism under the usual operations of addition
and multiplication since $f(2+3)=2^{2+3}=2^{2}2^{3}=32\neq12=2^{2}+2^{3}=f(2)+f(3)$.
...
$f(0)=f(n)=0\bmod n$. Also note that $|\mathbb{Z}|\neq|\mathbb{Z}_{n}|$
so there is no isomorphism between them.
...
\textbf{Presentation Problems due: \formatdate{11}{2}{2014}.}

Choose any one of the problems from chapter 2 of the notes.\vfill

\textbf{Presentation Problems due: \formatdate{18}{2}{2014}}

Choose 1 problem from exercises 2.9-2.12 in the notes or the following:
\begin{enumerate}
\item Prove that a ring that is cyclic under addition is commutative. 
\item Prove that $\mathbb{Z}_{p}$ is an integral domain if and only if
$p$ is prime (you can assume that it is a commutative ring with identity). 
