...
\end{example}
\begin{itemize}
\item []Let $a,b\in\mathbb{Z}$. Suppose that $b\neq0$ (if $b=0$ then
the condition is meet) then $ab=0\Rightarrow a=\nicefrac{0}{b}=0$
.
\end{itemize}

\begin{example}
The ring $\mathbb{Z}[x]$, the polynomials with integer coefficients,
is an integral domain.
\end{example}
\begin{xca}
Show that $\mathbb{Z}_{6}$ is \emph{not }an integral domain. 
\end{xca}

\begin{xca}
Prove that $\mathbb{Z}_{n}$ is not an integral domain if $n$ is
not prime.
\end{xca}
\begin{itemize}
...
\begin{xca}
\label{prob-Zp-is-an-ID}Prove that $\mathbb{Z}_{p}$ is an integral
domain if $p$ is prime.
\end{xca}

\begin{xca}
Prove that $\mathbb{Q}$ is an integral domain.
\end{xca}

\begin{xca}
Prove that the ring from exercise \ref{ex-funct-are-ring}, $F$,
...
\end{itemize}
\begin{example}
The ring of $2\times2$ matrices with real entries, $M_{2\times2}(\mathbb{R})$,
is not an integral domain.
\end{example}
\begin{proof}
This ring has zero $\begin{bmatrix}\begin{array}{rr}
0 & 0\\
0 & 0
\end{array}\end{bmatrix}$ so the matrices $\begin{bmatrix}\begin{array}{rr}
1 & 1\\
1 & 1
\end{array}\end{bmatrix}$ and $\begin{bmatrix}\begin{array}{rr}
1 & 1\\
-1 & -1
\end{array}\end{bmatrix}$ are nonzero and $\begin{bmatrix}\begin{array}{rr}
1 & 1\\
1 & 1
...
\end{array}\end{bmatrix}$.
\end{proof}
When two nonzero elements in a ring have a product that is $0$, we
call the elements \emph{zero divisors}. Having no such elements allows
for cancellation by the following theorem.
\begin{thm}
...
definition.
\begin{xca}
Let $a$ and $b$ be two nonzero elements in a ring $R$. Prove that
in general, $ax=b$ does not have a unique solution.
\end{xca}
\begin{itemize}
\item [Hint]It should be understood that $x$ must be in $R$. 
\end{itemize}
\begin{defn}
...
\end{itemize}
\begin{example}
$\mathbb{R}$ and $\mathbb{Q}$ are fields. 
\end{example}

\begin{example}
The set of \emph{constructable numbers}, numbers which can be constructed
with a straight edge and compass, is a field.
\end{example}
\begin{xca}
