...
\begin{axis}[ 
	...
	%ylabel=Frequency, 
	height=4cm, %change/remove this scaling to get 1:1 pic
	width=6cm, 
	hide y axis,
	%ticks=none,
	%xtick={\pgfmathparse{#1-3*#2}\pgfmathresult},\pgfmathparse{#1-2*#2}\pgfmathresult},\pgfmathparse{#1-1*#2}\pgfmathresult},\pgfmathparse{#1}\pgfmathresult},\pgfmathparse{#1+1*#2}\pgfmathresult},\pgfmathparse{#1+2*#2}\pgfmathresult},\pgfmathparse{#1+3*#2}\pgfmathresult}},
	...
	\addplot [draw, thick, smooth, domain=\pgfmathparse{#1-3*#2}\pgfmathresult:\pgfmathparse{#1+3*#2}\pgfmathresult] {\gauss{#1}{#2}} \closedcycle;
...
\begin{enumerate}
\item $p(z<1.75)$\vfill
\item $p(0<z<1.75)$\vfill
\item $p(-.5<z<1.75)$\vfill
\item $p(-1.5<z<-.5)$\vfill\newpage
\end{enumerate}
