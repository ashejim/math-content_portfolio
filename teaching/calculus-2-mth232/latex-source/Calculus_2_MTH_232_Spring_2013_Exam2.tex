...
\begin{axis}[height=6cm, 
	...
	\addplot+[domain=0:2,fill,black!30!white]{4*sqrt(2*x)} \closedcycle;
	\addplot+[domain=-1:3,fill,white]{-4*x+6} \closedcycle; 
	\addplot+[domain=-1:3,fill,white]{2*x^2}  \closedcycle;
	
	\addplot[domain=0:3,thick,smooth,samples=500]{4*sqrt(2*x)} node[anchor=135] {$f(x)=\dfrac{1}{x}$}; 
	\addplot[domain=-1:3,thick] {-4*x+6} node[right] {$f(x)=\dfrac{2}{x}$}; 	
	\addplot[domain=-1:3,thick] {2*x^2} node[right] {$f(x)=x$}; 
